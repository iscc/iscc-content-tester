...
# Get a list of available primary keys
available_primary_keys = [key for key in data if os.path.isdir(os.path.join(src_dir, key))]

total_videos = sum(len(data[key][sub_key]) for key in available_primary_keys for sub_key in data[key])
pbar = tqdm(total=total_videos, desc="Copying videos")

for key in available_primary_keys:
